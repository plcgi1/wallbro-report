...
function save($dbh){
    ...
    // проверим наличие записи в report
    if( isset($param->id) ){
        // записи нет
        $sth = $dbh->prepare('SELECT * FROM report WHERE id=?');
        $sth->execute(array($param->id));
        $report = $sth->fetch();
        // записи нет - возвращаем status : error, message: 'No such report'
        if( !isset($report) ) {
    ...
    if( isset($id) ) {
        ...
        $id = $dbh->lastInsertId();
        // перекинуть файлы из временной папки для хранения файлов - в папку CONFIG[UPLOAD_PATH]/id
    }
...
function upload($dbh){
    ...
    if ( !empty( $_FILES ) ) {
        ...
        // добавим запись в БД с инфой о файле
        $stmt = $dbh->prepare(
